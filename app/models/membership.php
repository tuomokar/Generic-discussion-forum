...
class Membership extends BaseModel {
    ...
    public function __construct($attributes) {
        ...
        $this -> validators = array('validateIds');
    }
    ...
    public function validateIds() {
        $errors = $this->validateFieldsExist(array('User' => $this->userId, 'Group' => $this->groupId));
        if ($errors) {
            return $errors;
        }

        $query = DB::connection()->prepare('SELECT u.id AS user_id, g.id AS group_id FROM forum_user u, topic_group g WHERE u.id = :userId OR g.id = :groupId');
        $query->execute(array('userId' => $this -> userId, 'groupId' => $this -> groupId));
